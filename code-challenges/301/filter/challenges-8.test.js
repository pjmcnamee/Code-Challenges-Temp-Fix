...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(item => {
      let didntFindTheNumber = true;
    forbiddenValues.forEach(num => {
          if(num === item){
            didntFindTheNumber = false;
          }

      }); return didntFindTheNumber;
...
const getStatName = (arr, minBaseStat) => {
  let tempArr = getBaseStatGreaterThan(arr, minBaseStat)
  let tempNamesArr = [];
  tempArr.forEach(data => {
    tempNamesArr.push(data.stat.name);
  })
  return tempNamesArr;
};
